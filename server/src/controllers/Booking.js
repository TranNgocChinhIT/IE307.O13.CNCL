...
export const createBooking = async (req, res) => {
  try {
    ...
    const existingMovieScheduleRelationship =
      await MovieScheduleRelationship.findById(movieScheduleRelationship);
    ...
    if (req.body.paymentStatus === "pending") {
      ...
      const updatedSeats = await updateSeatsStatusPending(
        movieScheduleRelationshipId,
...
const updateSeatsStatusPending = async (
  movieScheduleRelationshipId,
  selectedSeats
) => {
  try {
    const movieScheduleRelationship = await MovieScheduleRelationship.findById(
      movieScheduleRelationshipId
    );
    ...
    for (const seatId of selectedSeats) {
      const seatIndex = movieScheduleRelationship.seats
        .flat()
        .findIndex((seat) => seat._id == seatId);
      if (seatIndex !== -1) {
...
const updateSeatsStatusCanceled = async (
  movieScheduleRelationshipId,
  selectedSeats
) => {
  try {
...
export const updateBooking = async (req, res) => {
  try {
    ...
    if (req.body.paymentStatus === "completed") {
      const movieScheduleRelationshipId = req.body.movieScheduleRelationship;
      const updatedSeats = await updateSeatsStatus(
        movieScheduleRelationshipId,
        req.body.selectedSeats
      );
      if (!updatedSeats) {
      ...
      const movieScheduleRelationshipId = req.body.movieScheduleRelationship;
      const updatedSeats = await updateSeatsStatusCanceled(
        movieScheduleRelationshipId,
        req.body.selectedSeats
      );
      if (!updatedSeats) {
...
const autoDeletePendingBookings = async () => {
  try {
    ...
    const pendingBookings = await Booking.find({
      ...
      createdAt: { $lt: new Date(Date.now() - 3 * 60 * 1000) }, // Lọc các đặt vé tạo trước 1 ngày
    });

    // Xóa các đặt vé thỏa mãn điều kiện
    for (const booking of pendingBookings) {
      const movieScheduleRelationshipId =
        booking.movieScheduleRelationship.toString();
...
const runOnServerStart = async () => {
  console.log(
    "Server bắt đầu. Chạy hàm cập nhật xoa ve pending qua 1 ngay lần đầu tiên."
  );
