  ...
  const BookSeats = async () => {
    try {
      ...
      const selectedSeats = new Set(
        putData.selectedSeats.map((seat) => seat.toString())
      );
    ...
    <View style={styles.container}>
      <View style={{ flex: 0.25, marginTop: 30, marginBottom: 25 }}>
        <View style={{ flexDirection: "row" }}>
          <Image
            style={styles.image}
            source={{ uri: ticketData.note.imagePath }}
          ></Image>
          ...
          <View style={{ flexDirection: "column", marginLeft: 10 }}>
            ...
            <Text>
              {ticketData?.date &&
                `${ticketData.date.day}, ${ticketData.date.date}/${ticketData.month}/${ticketData.year}`}
            </Text>
            <Text>Time: {ticketData.time}</Text>
            <Text>
              Seats:{" "}
              {ticketData?.seatArray.slice(0, 80).map((item, index, arr) => (
      ...
      <ScrollView style={{ flex: 0.55 }}>
        ...
        <View>
          <View style={styles.inforContainer}>
            <Text style={styles.txtInfor}>Quantity</Text>

            <Text style={styles.txtQuantity}>{ticketData.quantity}</Text>
          </View>
          <View style={styles.separator}></View>
          <View style={styles.inforContainer}>
            <Text style={styles.txtInfor}>Sub Total</Text>

            <Text style={styles.txtQuantity}>{ticketData.total}.000 đ</Text>
          </View>
          <View style={styles.separator}></View>
        </View>
        ...
        <View>
          ...
          <View style={styles.inforContainer}>
            ...
            <Text style={styles.txtQuantity}>0.000 đ</Text>
          </View>
          <View style={styles.separator}></View>
        </View>
        <View style={{ backgroundColor: "#A9A9A9", with: "100%", height: 40 }}>
        ...
        <View>
          <View style={styles.inforContainer}>
            <Text style={styles.txtInfor}>Total including VAT</Text>

            <Text style={styles.txtQuantity}>{ticketData.total}.000 đ</Text>
          </View>
          <View style={styles.separator}></View>
          <View style={styles.inforContainer}>
            <Text style={styles.txtInfor}>Discount</Text>

            <Text style={styles.txtQuantity}>0 đ</Text>
          </View>
          <View style={styles.separator}></View>
          <View style={styles.inforContainer}>
            <Text style={styles.txtInfor}>After Discount</Text>
        ...
        <View>
          <TouchableOpacity onPress={handleMomoSelection}>
            <View style={styles.inforContainer}>
              <Image
                source={require("../assets/image/momo2.webp")}
                style={styles.imageIcon}
              ></Image>
              {isMomoSelected && (
                <Text style={{ color: "#FF3333", marginRight: 10 }}>✔</Text>
              )}
            </View>
          </TouchableOpacity>
